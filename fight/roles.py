...
class KaiWen(RoleBase):
    def skill_action(self):
        self.attack += 5
        self.ap += 25
        text = "凯文-清凉一剑触发,攻击力加5,造成25点魔法伤害"
...
class GeLeiXiu(RoleBase):
    ...
    def cal_harmed(self):  # type: ignore
        ...
        if self.shield > 0:
            self.harmed_rest = (
                self.harmed - self.shield if self.harmed - self.shield > 0 else 0
            )
...
class Sakura(RoleBase):
    ...
    def skill_action(self):
        ...
        text = f"樱-夏之型-瓜切发动,回复了{rand}点血量, 对敌方造成{self.attack * 130 // 100}点物理伤害"  # noqa: E501
        self.text_handle(text)
...
class Hua(RoleBase):
    def skill_action(self):
        ...
        def update_attack_normal(self: "RoleBase", func):
            def wrapper():
                ...
                text = f"华蓄力完成,防御力恢复,当前为{self.defence},攻击附加{rand}点魔法伤害(无视混乱)"  # noqa: E501
                self.text_handle(text)
                func()
                self.ap += rand
    ...
    def cal_harmed(self):
        super().cal_harmed()
        self.harmed = int(self.harmed * 0.8 // 1)
    ...
    def skill_action(self):
        ...
        for i in range(4):
            ...
            if self.enemy.tear > 0:
                self.ap += 3
            self.enemy.harmed += (
                rand - self.enemy.defence if rand - self.enemy.defence > 0 else 0
            )
        if self.enemy.tear > 0:
            text += "由于对手陷入撕裂状态,分别附加3点魔法伤害"
        self.text_handle(text)

    def skill_passive(self):
        text = "科斯魔-不归之爪触发,造成撕裂效果"
        if self.chaos:
            text += "但由于混乱,返还自身"
            self.tear = 3
        else:
...
class MeiBiWuSi(RoleBase):
    ...
    def skill_passive(self):
        ...
        if self.chaos:
            ...
            self.enemy.defence = (
                self.enemy.defence - 2 if self.enemy.defence - 2 > 0 else 0
            )
...
class QianJie(RoleBase):
    def skill_action(self):
        if self.blood >= 11:
            ...
            self.ap += rand
            self.action_num -= 1
            text = f"千劫-盛夏燔祭发动,血量-10,当前为{self.blood},造成45点物理伤害并附带{rand}点魔法伤害并休息一回合"  # noqa: E501
            self.text_handle(text)
    ...
    def skill_action(self):
        ...
        def update_enemy_cal_harmed(self: "RoleBase", func):
            def wrapper():
                ...
                if temp and blood_recovery == 0:
                    blood_recovery = temp
                self.enemy.blood = (
                    self.enemy.blood + blood_recovery
                    if self.enemy.blood + blood_recovery < 100
                    else 100
                )

                if blood_recovery > 0:
                    text = f"帕朵菲利斯-沙滩寻宝效果,恢复{blood_recovery}点血量,当前血量{self.enemy.blood}"  # noqa: E501
                    self.text_handle(text)
                self.cal_harmed = self.original_funcs["paduofeilisi>cal_harmed"]

            return wrapper

        self.enemy.cal_harmed = update_enemy_cal_harmed(
            self.enemy, self.enemy.cal_harmed
        )
        self.cal_and_trigger()

    def skill_passive(self):
        text = "帕朵菲利斯-最佳搭档触发,造成30点物理伤害"
...
class YiDian(RoleBase):
    ...
    def skill_action(self):
        ...
        def update_turn_begin(self: "YiDian", func):
            def wrapper():
                func()
