...
class RoleBase:
    ...
    def __init__(
        ...
        self.id = id
        self.type_p = type_p
        self.original_funcs: dict[str, Callable] = {}
        self.txts = []
    ...
    def judge_winner(self):
        if self.blood <= 0 and not self.result.get("winner_name"):
            self.result["winner_name"] = self.enemy.name

    def cal_harmed(self):  # 计算伤害
        self.harmed += self.enemy.ap
        self.harmed += max(self.enemy.ad - self.defence, 0)
        self.enemy.ad = 0
        self.enemy.ap = 0
